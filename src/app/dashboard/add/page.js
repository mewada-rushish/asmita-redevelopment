...
export default function AddPropertyPage() {
  ...
  const [formData, setFormData] = useState({
    type: '', category: '',
    pin_code: '', latitude: null, longitude: null,
  ...
  useEffect(() => {
    ...
    if (formData.has_approved_plan === 1 || formData.has_cc === 1 || formData.approved_plan_file || formData.cc_file) maxIndex = Math.max(maxIndex, 10);
    else if (formData.da_agreement_status === 'In Process' || formData.da_agreement_status === 'Completed') maxIndex = Math.max(maxIndex, 9);
    else if (formData.consent_79a_file || formData.consent_type === '100%') maxIndex = Math.max(maxIndex, 8);
    else if (formData.offer_acceptance_letter_file || formData.offer_letter_status === 'Accepted') maxIndex = Math.max(maxIndex, 7);
    else if (formData.offer_letter_status === 'Under Negotiation' || (Array.isArray(formData.activity_logs) && formData.activity_logs.some(l => l.category === 'Offer Negotiation'))) maxIndex = Math.max(maxIndex, 6);
    else if ((Array.isArray(formData.offer_letter_files) && formData.offer_letter_files.length > 0) || formData.offer_letter_sent === 1) maxIndex = Math.max(maxIndex, 5);
    else if (formData.architect_survey_status === 'Completed') maxIndex = Math.max(maxIndex, 4);
    else if (formData.architect_survey_status === 'Started' || formData.sent_to_architect === 1) maxIndex = Math.max(maxIndex, 3);
    ...
    setFormData(prev => {
      const currentStatusIndex = STATUS_FLOW.indexOf(prev.status);
      if (maxIndex > currentStatusIndex) {
        return { ...prev, status: STATUS_FLOW[maxIndex] };
      }
      return prev;
    });
  }, [
